/* *****************************************************************************
 * @brief FKMG LED Control implementation.
 * 
 */
...
#include "io-ctrl/led/led.h"
#include <zephyr/smf.h>
#include <zephyr/kernel.h>
#include <zephyr/drivers/gpio.h>
#include <assert.h>
#include <stdlib.h>
#include "io-ctrl/led/private/sm_evt.h"
#include "io-ctrl/led/private/module_data.h"
...
LOG_MODULE_REGISTER(fkmg_io_ctrl_led);

/* *****************************************************************************
 * Structs
 */

static struct fkmg_led_module_data led_md = {0};
#define md led_md
...
static struct FKMG_IO_CTRL_LED_Instance * sm_ctx_to_instance(struct smf_ctx * p_sm_ctx)
{
    /* Iterate thru the instances. */
    struct FKMG_IO_CTRL_LED_Instance * p_inst = NULL;
...
static void clear_listener(struct FKMG_IO_CTRL_LED_Listener * p_lsnr)
{
    memset(p_lsnr, 0, sizeof(*p_lsnr));
}

static void config_listener(
        struct FKMG_IO_CTRL_LED_Listener     * p_lsnr,
        struct FKMG_IO_CTRL_LED_Listener_Cfg * p_cfg)
{
...
static void init_listener(struct FKMG_IO_CTRL_LED_Listener * p_lsnr)
{
...
static void config_instance_queues(
        struct FKMG_IO_CTRL_LED_Instance     * p_inst,
        struct FKMG_IO_CTRL_LED_Instance_Cfg * p_cfg)
{
...
static void init_instance_lists(struct FKMG_IO_CTRL_LED_Instance * p_inst)
{
    for(enum FKMG_IO_CTRL_LED_Evt_Sig sig = k_FKMG_IO_CTRL_LED_Evt_Sig_Beg;
                         sig < k_FKMG_IO_CTRL_LED_Evt_Sig_End;
                         sig++){
        sys_slist_init(&p_inst->list.listeners[sig]);
    }
}

static void clear_instance(struct FKMG_IO_CTRL_LED_Instance * p_inst)
{
    memset(p_inst, 0, sizeof(*p_inst));
}

static void init_instance(struct FKMG_IO_CTRL_LED_Instance * p_inst)
{
...
static void broadcast(
        struct FKMG_IO_CTRL_LED_Evt      * p_evt,
        struct FKMG_IO_CTRL_LED_Listener * p_lsnr)
{
...
static void broadcast_event_to_listeners(
        ...
        struct FKMG_IO_CTRL_LED_Evt      * p_evt)
{
    enum FKMG_IO_CTRL_LED_Evt_Sig sig = p_evt->sig;
    struct FKMG_IO_CTRL_LED_Listener * p_lsnr = NULL;
...
static void broadcast_instance_initialized(
        struct FKMG_IO_CTRL_LED_Instance * p_inst,
        FKMG_IO_CTRL_LED_Listener_Cb       cb)
{
    struct FKMG_IO_CTRL_LED_Evt evt = {
            .sig = k_FKMG_IO_CTRL_LED_Evt_Sig_Instance_Initialized,
            .data.initd.p_inst = p_inst
    };

    struct FKMG_IO_CTRL_LED_Listener lsnr = {
        .p_inst = p_inst,
        .cb = cb
    };
        ...
        uint16_t              val)
{
    ...
    struct FKMG_IO_CTRL_LED_Evt evt = {
            .sig = k_FKMG_IO_CTRL_LED_Evt_Sig_Changed,
            .data.changed.id = id,
    ...
    struct FKMG_IO_CTRL_LED_Listener_Cfg * p_cfg)
{
    ...
    struct FKMG_IO_CTRL_LED_Instance * p_inst = p_cfg->p_inst;

    /* Get pointer to configured listener. */
    struct FKMG_IO_CTRL_LED_Listener * p_lsnr = p_cfg->p_lsnr;

    /* Get signal to listen for. */
    enum FKMG_IO_CTRL_LED_Evt_Sig sig = p_cfg->sig;
...
static void q_sm_event(struct FKMG_IO_CTRL_LED_Instance * p_inst, struct FKMG_IO_CTRL_LED_SM_Evt * p_evt)
{
...
static void q_init_instance_event(struct FKMG_IO_CTRL_LED_Instance_Cfg * p_cfg)
{
    struct FKMG_IO_CTRL_LED_SM_Evt evt = {
            .sig = k_FKMG_IO_CTRL_LED_SM_Evt_Sig_Init_Instance,
...
static int LED_Array_Init(struct FKMG_IO_CTRL_LED_Instance *p_inst) 
{
    int ret = -1; 
...
static void state_init_run(void * o)
{
    ...
    struct FKMG_IO_CTRL_LED_Instance * p_inst = sm_ctx_to_instance(p_sm);

    /* Get the event. */
    struct FKMG_IO_CTRL_LED_SM_Evt * p_evt = &p_inst->sm_evt;

    /* Expecting only an "init instance" event. Anything else is an error. */
    assert(p_evt->sig == k_FKMG_IO_CTRL_LED_SM_Evt_Sig_Init_Instance);
    ...
    struct FKMG_IO_CTRL_LED_SM_Evt_Sig_Init_Instance * p_ii = &p_evt->data.init_inst;
...
static void state_run_run(void * o)
{
    ...
    switch(p_evt->sig){
        default: break;
        case k_FKMG_IO_CTRL_LED_SM_Evt_Sig_Init_Instance:
            /* Should never occur. */
            assert(false);
            break;
        case k_FKMG_IO_CTRL_LED_SM_Evt_Sig_Write:
            
            struct FKMG_IO_CTRL_LED_SM_Evt_Sig_Write * p_convert = &p_evt->data.write;
...
static void thread(void * p_1, /* struct FKMG_IO_CTRL_LED_Instance* */
        void * p_2_unused, void * p_3_unused)
{
    struct FKMG_IO_CTRL_LED_Instance * p_inst = p_1;
...
void FKMG_IO_CTRL_LED_Write_LED(struct FKMG_IO_CTRL_LED_Instance * p_inst, enum FKMG_IO_CTRL_LED_Id id, bool status)
{
    ...
    // struct FKMG_IO_CTRL_LED_SM_Evt evt = {
    //         .sig = k_FKMG_IO_CTRL_LED_SM_Evt_Sig_Write,
    //         .data.write.row = row,
    //         .data.write.val = val
    // };
    // q_sm_event(p_inst, &evt);
