/* *****************************************************************************
 * @brief FKMG I/O Control DAC implementation.
 * 
 */
...
#include "io-ctrl/dac/private/module_data.h"
...
LOG_MODULE_REGISTER(fkmg_io_ctrl_dac);

/* *****************************************************************************
 * Structs
 */

static struct fkmg_dac_module_data fkmg_dac_md = {0};
#define md fkmg_dac_md
...
static struct FKMG_IO_CTRL_DAC_Instance * sm_ctx_to_instance(struct smf_ctx * p_sm_ctx)
{
    /* Iterate thru the instances. */
    struct FKMG_IO_CTRL_DAC_Instance * p_inst = NULL;
...
static enum FKMG_IO_CTRL_DAC_Id next_dac_channel(enum FKMG_IO_CTRL_DAC_Id id)
{
    return((id + 1) % k_FKMG_IO_CTRL_DAC_Id_Cnt);
}
...
static void clear_listener(struct FKMG_IO_CTRL_DAC_Listener * p_lsnr)
{
...
static void config_listener(
        ...
        struct FKMG_IO_CTRL_DAC_Listener_Cfg * p_cfg)
{
...
static void init_listener(struct FKMG_IO_CTRL_DAC_Listener * p_lsnr)
{
...
static void config_instance_queues(
        struct FKMG_IO_CTRL_DAC_Instance     * p_inst,
        struct FKMG_IO_CTRL_DAC_Instance_Cfg * p_cfg)
{
...
static void init_conversion_timer(struct FKMG_IO_CTRL_DAC_Instance * p_inst)
{
...
/* Forward reference */
static int init_io_ctrl_dac_device(struct FKMG_IO_CTRL_DAC_Instance * p_inst); 

static void config_instance_deferred(
        struct FKMG_IO_CTRL_DAC_Instance     * p_inst,
        struct FKMG_IO_CTRL_DAC_Instance_Cfg * p_cfg)
{
    p_inst->dac_t_duration_ms = p_cfg->dac_t_duration_ms;
    p_inst->dac_t_period_ms = p_cfg->dac_t_period_ms;
    init_io_ctrl_dac_device(p_inst);
}
...
static void init_instance_lists(struct FKMG_IO_CTRL_DAC_Instance * p_inst)
{
    for(enum FKMG_IO_CTRL_DAC_Evt_Sig sig = k_FKMG_IO_CTRL_DAC_Evt_Sig_Beg;
                         sig < k_FKMG_IO_CTRL_DAC_Evt_Sig_End;
                         sig++){
        sys_slist_init(&p_inst->list.listeners[sig]);
    }
}

static void clear_instance(struct FKMG_IO_CTRL_DAC_Instance * p_inst)
{
    memset(p_inst, 0, sizeof(*p_inst));
}

static void init_instance(struct FKMG_IO_CTRL_DAC_Instance * p_inst)
{
...
static void broadcast(
        struct FKMG_IO_CTRL_DAC_Evt      * p_evt,
        struct FKMG_IO_CTRL_DAC_Listener * p_lsnr)
{
...
static void broadcast_event_to_listeners(
        ...
        struct FKMG_IO_CTRL_DAC_Evt      * p_evt)
{
    enum FKMG_IO_CTRL_DAC_Evt_Sig sig = p_evt->sig;
    struct FKMG_IO_CTRL_DAC_Listener * p_lsnr = NULL;
...
{
    struct FKMG_IO_CTRL_DAC_Evt evt = {
            .sig = k_FKMG_IO_CTRL_DAC_Evt_Sig_Instance_Initialized,
            .data.initd.p_inst = p_inst
    };

    struct FKMG_IO_CTRL_DAC_Listener lsnr = {
        .p_inst = p_inst,
        .cb = cb
    };
    ...
    struct FKMG_IO_CTRL_DAC_Listener_Cfg * p_cfg)
{
    ...
    struct FKMG_IO_CTRL_DAC_Instance * p_inst = p_cfg->p_inst;

    /* Get pointer to configured listener. */
    struct FKMG_IO_CTRL_DAC_Listener * p_lsnr = p_cfg->p_lsnr;

    /* Get signal to listen for. */
    enum FKMG_IO_CTRL_DAC_Evt_Sig sig = p_cfg->sig;
...
static void q_sm_event(struct FKMG_IO_CTRL_DAC_Instance * p_inst, struct FKMG_IO_CTRL_DAC_SM_Evt * p_evt)
{
...
static void q_init_instance_event(struct FKMG_IO_CTRL_DAC_Instance_Cfg * p_cfg)
{
    struct FKMG_IO_CTRL_DAC_SM_Evt evt = {
            .sig = k_FKMG_IO_CTRL_DAC_SM_Evt_Sig_Init_Instance,
...
{
    struct FKMG_IO_CTRL_DAC_Instance * p_inst =
        CONTAINER_OF(p_timer, struct FKMG_IO_CTRL_DAC_Instance, timer.conversion);

    enum FKMG_IO_CTRL_DAC_Id nextDACId = next_dac_channel(p_inst->id);
    p_inst->id = nextDACId;
    q_convert(p_inst, nextDACId);

}

/* Cause conversion event to occur immediately and then regularly after that. */
static void start_conversion_timer(struct FKMG_IO_CTRL_DAC_Instance * p_inst)
{
    #define CONVERSION_INITIAL_DURATION     K_MSEC(p_inst->dac_t_duration_ms)
    #define CONVERSION_AUTO_RELOAD_PERIOD   K_MSEC(p_inst->dac_t_period_ms)
...
static void state_init_run(void * o)
{
    ...
    struct FKMG_IO_CTRL_DAC_Instance * p_inst = sm_ctx_to_instance(p_sm);

    /* Get the event. */
    struct FKMG_IO_CTRL_DAC_SM_Evt * p_evt = &p_inst->sm_evt;

    /* Expecting only an "init instance" event. Anything else is an error. */
    assert(p_evt->sig == k_FKMG_IO_CTRL_DAC_SM_Evt_Sig_Init_Instance);
    ...
    struct FKMG_IO_CTRL_DAC_SM_Evt_Sig_Init_Instance * p_ii = &p_evt->data.init_inst;
...
static void state_run_run(void * o)
{
    ...
    switch(p_evt->sig){
        default: break;
        case k_FKMG_IO_CTRL_DAC_SM_Evt_Sig_Init_Instance:
            /* Should never occur. */
            assert(false);
            break;
        case k_FKMG_IO_CTRL_DAC_SM_Evt_Sig_Write_Sample:

            /* Write the sample to the DAC. */
            int ret = dac_write_value(dac_dev, 
                                      DAC_CHANNEL_ID, 
                                      p_evt->data.write.sample);
...
static void thread(void * p_1, /* struct DAC Instance* */
        void * p_2_unused, void * p_3_unused)
{
    struct FKMG_IO_CTRL_DAC_Instance * p_inst = p_1;
...
void FKMG_IO_CTRL_DAC_Write_Sample(struct FKMG_IO_CTRL_DAC_Instance * p_inst,
                                   enum FKMG_IO_CTRL_DAC_Id id,
                                   uint16_t sample)
{
    struct FKMG_IO_CTRL_DAC_SM_Evt evt = {
            .sig = k_FKMG_IO_CTRL_DAC_SM_Evt_Sig_Write_Sample,
            .data.write.id = id,
